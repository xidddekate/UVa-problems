...
int main(){
    ...
    cin.tie(NULL);int m=INT_MIN;
while(cin>>t>>n>>k)
{m=max(m,k);FORL(i,t,k-1)if(a[i]<n)a[i]=n;}int i=1;if(a[i]==a[i-1])cout<<1<<" "<<0<<endl;while(i<=m){if(a[i]==a[i-1])while(a[i]==a[i-1])i++;else {cout<<i<<" "<<a[i]<<" ";i++;}}cout<<endl;
// FOR(i,m){cout<<a[i]<<" ";}
return 0;
}                     