...
{
    // cout<<t<<endl;
    string s[t];FOR(i,t)cin>>s[i];
    
    // FOR(i,t)cout<<s[i];
    vpi three,one;
    FOR(i,t)FOR(j,t)if(s[i][j]=='1')one.pb({i,j});else if(s[i][j]=='3')three.pb({i,j});k=INT_MIN;
    // FOR(i,one.size()){cout<<one[i].F<<" ";cout<<endl;}
    FOR(i,one.size()){n=INT_MAX;FOR(j,three.size())n=min(abs(one[i].F-three[j].F)+abs(one[i].S-three[j].S),n);k=max(k,n);}
    println(k);
}
