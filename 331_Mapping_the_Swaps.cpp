...
int a[5];map<int,int>m;

bool isSorted()
{int m=INT_MIN;FOR(i,n)if(m<a[i])m=a[i];else return false;return true;}

void rec(int c)
{
    if(isSorted()){m[c]+=1,t=min(t,c);return;}
    else
    FOR(i,n-1)if(a[i]>a[i+1]){swap(a[i],a[i+1]);rec(c+1);swap(a[i],a[i+1]);}
...
while (cin>>n&&n)
{
    memset(a,0,sizeof(a));FOR(i,n)cin>>a[i];t=INT_MAX;if(!isSorted())rec(0);k++;cout<<"There are "<<m[t]<<" swap maps for input data set "<<k<<"."<<endl;m.clear();
}
