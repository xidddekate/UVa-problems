...
int main()
{
    ...
    while( scanf( "%d.%d %d.%d", &a, &b, &c, &d ) != EOF )
    {
        ...
        for( int i = low; i <= high; ++i )
            if( !isprime[ i ] )
                for( int p = 1; p < i && p*p*p <= i * 10000; ++p ) // as 1.32*3.43*4.44=abc.xy
                    if( i * 10000 % p == 0 )
                        for( int q = p; p*q*q <= i*10000; ++q )
                            if( i*10000/p % q == 0 )
                            {
                                ...
                                if( p + q + r == i && p * q * r == i*10000 )
                                    printf("%d.%02d = %d.%02d + %d.%02d + %d.%02d = %d.%02d * %d.%02d * %d.%02d\n", i/ 100,i% 100, p/ 100,p% 100 , q/ 100,q% 100 , r/ 100,r%100, p/ 100,p% 100 , q/ 100,q% 100 , r/ 100,r%100 ), ++all;
                            }
    }
    return 0;
}
...
// MORE FASTER (as solutions are less so precompute) :-
